...
因此我们需要**通过Nacos去获取TC服务**

那么什么微服务需要TC监管呢，答案当然业务微服务（注意！没有事务需求就别引入）

**本来是要把Seata配置文件写入Nacos去获取的，但是未知原因，无法拉取配置文件，因此这里不作笔记，相信后人智慧**

1. 当确定进入Seata的控制台后，并且**能从Nacos看见Seata的服务**后，那么得导入依赖
...
2. 导入完成后，到**SpringBoot的配置文件**中加入这段代码
...
3. 然后启动后注意观察**Seata的窗口**![控制台](../../图床/92fcafc5-d5c4-40b9-b172-fe04149e734e.png)当你看见命令行窗口有你的项目名，就**成功连入**

4. 既然是事务，当然**集成在一个微服务上是不够的**，得多弄几个，这样才完整
...
3. 然后把配置文件中的**XA模式换成AT**就可以了
   ...
   seata:
     ...
     service:
       vgroup-mapping:
         hmall: "default"                  #组名
     data-source-proxy-mode: XA            #开启事务AT模式
   ```
